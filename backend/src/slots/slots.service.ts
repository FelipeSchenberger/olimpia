...
export class SlotsService {
  ...
  async getSlotsForDate(dateStr: string, courtId: number = 1) {
    ...
    if (countMain < expectedRegularHours || countNight < 2) {
        console.log(`Incomplete slots for ${dateStr} Court ${courtId} (Main: ${countMain}, Night: ${countNight}). Filling gaps...`);
  ...
  async createFixedSlot(data: any) {
      ...
      console.log(`[CreateFixed] Request for Day ${dayOfWeek}, Time ${data.startTime}, Court ${courtId}`);
      ...
      console.log(`[CreateFixed] Found ${candidates.length} future candidates matching time.`);
      ...
      console.log(`[CreateFixed] Identified ${idsToUpdate.length} slots to update to FIXED.`);
  ...
  private async createSlotIfNotExists(date: Date, time: string, courtId: number, fixedSlots: any[]) {
  ...
  async updateSlotStatus(id: number, status: string, clientName?: string, type?: string) {
